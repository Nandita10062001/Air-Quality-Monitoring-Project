...
def calculate_mae_for_k(
    ...
    # Drop all the rows with the stations where the data imputation didn't perform well
    ...
    for station in stations:
        ...
        if len(df_day_station) > 0:
            ...
            if len(df_day_no_station) >= k:
                ...
                df_day_no_station[['Latitude', 'Longitude', 'time_discriminator']] = df_day_no_station[['Latitude', 'Longitude', 'time_discriminator']].apply(pd.to_numeric, errors='coerce')
                df_day_station[['Latitude', 'Longitude', 'time_discriminator']] = df_day_station[['Latitude', 'Longitude', 'time_discriminator']].apply(pd.to_numeric, errors='coerce')

                # Fit the KNN model
                neigh = KNeighborsRegressor(n_neighbors=k, weights='distance', metric='euclidean')
                knn_model = neigh.fit(
                    df_day_no_station[['Latitude', 'Longitude', 'time_discriminator']],
                    df_day_no_station[[target_pollutant]]
                )
                
                # Make predictions only if valid data is available
                if not df_day_station[['Latitude', 'Longitude', 'time_discriminator']].isnull().values.any():
...
def create_animation_features(
    ...
    k_neighbors_model = KNeighborsRegressor(n_neighbors=k, weights='distance', metric='euclidean')
